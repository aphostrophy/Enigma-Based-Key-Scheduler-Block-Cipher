...
class EnigmaBytesMachine:
    ...
    def _advance_rotors(self):
        if chr(self.rotor_positions[1]) == EnigmaBytesRotor.get_turnover_notch(self.rotors[1]):
            print("Advance Var 2")
            self.rotor_positions[1] = (self.rotor_positions[1] + 1) % 256
            self.rotor_positions[0] = (self.rotor_positions[0] + 1) % 256
        if chr(self.rotor_positions[2]) == EnigmaBytesRotor.get_turnover_notch(self.rotors[2]):
            print("Advance Var 1")
            self.rotor_positions[1] = (self.rotor_positions[1] + 1) % 256
    ...
    def encrypt(self, message: bytes) -> bytes:
        ...
        for letter in message:
            ...
            print("initial plugboard", letter, bytes([letter]))
            letter = self._forward_substitution(letter)
            print("forward sub", letter, bytes([letter]))
            letter = self._reflector_substitution(letter)
            print("reflector sub", letter, bytes([letter]))
            letter = self._backward_substitution(letter)
            print("backward sub", letter, bytes([letter]))
            letter = self._plugboard_substitution(letter)
            print("final plugboard", letter, bytes([letter]))
